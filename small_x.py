    ...
    if os.path.isfile(market_f):
        ...
        for i in range(ids_count):
            ...
            if not table_part.isnull().any().any():
                ...
                if e - n > 0:
                    y = market.low[e - n: e].max() / np.mean([market.high.iloc[e + n],market.low.iloc[e + n]])
                else:
                    y = np.nan
                ...
                Y_elem = {'id': coin + str(i),
                          ...
                          'predicted': market.date.iloc[e] + datetime.timedelta(days=n),
        ...
        for elem in small_Y:
            small_Y['pumps'] = Y[small_Y['id']]['pumps']


small_x = pd.concat(small_x, ignore_index=True)
small_Y = pd.DataFrame(small_Y)
p = '_N'+str(n)+'L'+str(l)+'W'+str(w)
small_Y.to_csv('small_Y'+p+'.csv', index=False)
small_x.drop(skip_colums,axis=1).to_csv('small_X'+p+'.csv', index=False)
